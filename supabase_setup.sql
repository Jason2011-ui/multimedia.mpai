...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'stats', jsonb_build_object(
      ...
      'posts', (SELECT COUNT(*) FROM mm_social_posts WHERE user_id = p_user_id AND hidden = FALSE)
    ),
    'posts', COALESCE((
      ...
      FROM (SELECT * FROM mm_social_posts WHERE user_id = p_user_id AND hidden = FALSE ORDER BY created_at DESC LIMIT 8) p
    ), '[]'::jsonb)
  );
END;
$$;
